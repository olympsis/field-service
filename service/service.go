...
func NewFieldService(l *logrus.Logger, r *mux.Router) *FieldService {
	return &FieldService{Log: l, Router: r}
}
...
*/
func (f *FieldService) InsertAField() http.HandlerFunc {
...
*/
func (f *FieldService) GetAField() http.HandlerFunc {
	return func(rw http.ResponseWriter, r *http.Request) {
...
*/
func (f *FieldService) UpdateAField() http.HandlerFunc {
	return func(rw http.ResponseWriter, r *http.Request) {
		var req Field
...
*/
func (f *FieldService) DeleteAField() http.HandlerFunc {
	return func(rw http.ResponseWriter, r *http.Request) {
		// grab club id from path
		vars := mux.Vars(r)
		if len(vars["id"]) == 0 || len(vars["id"]) < 24 {
...
*/
func (f *FieldService) DecodeToken(token string) (jwt.MapClaims, error) {
...
Returns:
	...
	error -  if there is an error return error else nil
*/
func (f *FieldService) GrabToken(r *http.Request) (string, error) {
...
func (f *FieldService) Healthz() http.HandlerFunc {
	return func(rw http.ResponseWriter, r *http.Request) {
		...
		if f.Status == INIT {
			...
			status["status"] = OK
			json.NewEncoder(rw).Encode(status)
		} else if f.Status == DB_CONN {
			rw.WriteHeader(http.StatusInternalServerError)
			status["status"] = DB_CONN
			json.NewEncoder(rw).Encode(status)
		} else {
			rw.WriteHeader(http.StatusInternalServerError)
